...
hist(
  ...
  breaks = 50,
  main = "Reads per cell",
  xlab = "log(Total Reads)"
)
abline(v = c(log(2e4), log(3e6)), col = "red")

filter_by_total_counts <- (reads$total_counts > 2e4 & reads$total_counts < 3e6)
...
abline(v = c(800, 3000), col = "red")

filter_by_expr_features <- (reads$total_features > 800 & reads$total_features < 3000)
...
## Save to RDS file
```{r}
saveRDS(reads, file ="data/13moWTB_reads.rds")
saveRDS(reads_qc, file = "data/13moWTB_reads_qc.rds")
```
