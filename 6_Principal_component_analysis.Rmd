...
```{r}
rv <- rowVars(assay(sce_object))#fill in name of sce_object
selected_genes <- order(rv, decreasing = TRUE)[seq_len(min(ntop, length(rv)))]#Change ntop with amount of genes you want to do PCA on
pca <- prcomp(t(assay(sce_object)[selected_genes,]))#fill in name of sce_object
pca_loadings <- as.data.frame(pca$rotation)
pca_loadings[order(pca_loadings[,1], decreasing = TRUE),]
write.xlsx(pca_loadings, file_name)#fill in filename
```
